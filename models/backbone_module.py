...
from pointnet2_modules import PointnetSAModuleVotes, PointnetFPModule, ThreeNNInterpolate, PointnetSAModuleCenters
...
class Pointnet2BackboneWithSem(nn.Module):
    r"""
       Backbone network for point cloud feature learning.
       Based on Pointnet++ single-scale grouping network. 
    ...
    def __init__(self, input_feature_dim=0, width=1, depth=2):
        ...
        self.fc_layer = nn.Sequential(
            nn.Conv1d(896 * width, 512, kernel_size=1, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(True),
            nn.Dropout(0.5),
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, center_label: torch.cuda.FloatTensor, end_points=None):
        r"""
            Forward pass of the network
        ...
        # --------- Semantic Prediction ---------
        center_features_sa1 = ThreeNNInterpolate(end_points['sa1_features'], end_points['sa1_xyz'], center_label) # (B, C1, Nc)
        center_features_sa2 = ThreeNNInterpolate(end_points['sa2_features'], end_points['sa2_xyz'], center_label) # (B, C2, Nc)
        center_features_sa3 = ThreeNNInterpolate(end_points['sa3_features'], end_points['sa3_xyz'], center_label) # (B, C3, Nc)
        center_features_sa4 = ThreeNNInterpolate(end_points['sa4_features'], end_points['sa4_xyz'], center_label) # (B, C4, Nc)
        center_features = torch.cat([center_features_sa1, center_features_sa2, center_features_sa3, center_features_sa4], dim=1)
        center_pred = self.fc_layer(center_features) # (B, 22, Nc)
        end_points['center_sem_scores'] = center_pred
